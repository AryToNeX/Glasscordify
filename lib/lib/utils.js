...
const rootApps = require("./root_applications.json5");
const appBlacklist = require("./blacklist.json5");
...
class Utils{
		...
		let promise = await new Promise((resolve, reject) => {
			...
			this.httpsGet("https://api.github.com/repos/AryToNeX/Glasscord/releases/latest", {headers: {"user-agent": "glasscordify"}}, result => {
				...
				this.httpsGet(url, {headers: {"user-agent": "glasscordify"}}, file => {
					...
					// Save data
					const glasscordRoot = path.resolve(this.getConfigPath(), "glasscord");
					extraFs.ensureDirSync(path.resolve(glasscordRoot, "_bin"));
					originalFs.writeFileSync(path.resolve(glasscordRoot, "_bin", "glasscord._asar"), file.data);
					originalFs.renameSync(path.resolve(glasscordRoot, "_bin", "glasscord._asar"), path.resolve(glasscordRoot, "_bin", "glasscord.asar"))
					this._ensureChown(glasscordRoot);
					return resolve(true);
				});
			});
		});
	...
	/**
	 * Ensure the file is owned by the current user when Glasscordify is ran with sudo instead of root
	 * @param {PathLike} file Path to the file
	 * @private
	 */
