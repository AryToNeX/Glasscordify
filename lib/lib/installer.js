...
class Installer{
	...
	static checkApp(_appExecutablePath, options = {
		...
		if(typeof result.appPathType === "undefined") return {error: "App path not found"};
		...
		if(typeof result.appName === "undefined")
			result.appName = path.parse(_appExecutablePath).name;
		
		result.rootAppName = Utils.determineRootAppName(result.appName);

		if(result.appPathType !== "js")
			result.mainFile = Utils.determineMainFile(result.appPath);
		if(typeof result.mainFile === "undefined")
			return {error: "Can't determine the app's entry point"};
